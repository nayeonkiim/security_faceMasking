...
from opencv.decrypt import dec
from opencv.encrypt import enc
from opencv.img_change import get_img, print_hex_bytes, toImgFile, image_to_byte_array

def go_encrpt(filePath):
    # 이미지 파일 가져오기
    ...
    cv2.imwrite("human8.jpg", arr)


def go_decrypt(filePath, cipher_data, mac) :
    # 각종 키 정보
    key = bytes([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF])
    aad = bytes([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E])
    nonce = bytes(
    ...
    # 사진 복호화 완료
    filePath = filePath.split('.jpg');
    print(filePath[0] + '_decrypt.jpg');
    newFileName = filePath[0] + '_decrypt.jpg';
    cv2.imwrite(newFileName)